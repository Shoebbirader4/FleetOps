...
try {
  ...
  await check("Technician completes assigned work order using inventory", () => tRPC("workOrders.complete", technicianToken, { workOrderId, parts: [{ partId, qtyUsed: 1 }] }));
  ...
  const completed = await pool.query(`SELECT status FROM work_orders WHERE id = $1`, [workOrderId]); if (completed.rows[0].status !== "COMPLETED") throw new Error("Work order was not completed"); results.push({ name: "Verify completed work order and inventory consumption", status: "PASS", detail: "status COMPLETED and one inventory unit consumed" });
