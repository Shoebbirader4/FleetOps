...
export const appRouter = router({
  ...
  onboarding: router({
    ...
    acceptInvite: publicProcedure.input(z.object({ token: z.string().uuid(), fullName: z.string().min(2).optional() })).mutation(async ({ ctx, input }) => {
      const authUser = await getSupabaseAuthIdentity(ctx.req);
      if (!authUser?.email) throw new TRPCError({ code: "UNAUTHORIZED", message: "A valid Supabase access token is required." });
      const invite = await prisma.invitation.findFirst({ where: { tokenHash: input.token, email: authUser.email, acceptedAt: null, expiresAt: { gt: new Date() } } });
  ...
  dashboard: router({
    summary: fleetOpsProcedure.query(async ({ ctx }) => {
      ...
      const [vehicles, openWorkOrders, inventoryAlerts, unreadNotifications, spend] = await Promise.all([
        prisma.vehicle.findMany({ where: { orgId }, orderBy: { updatedAt: "desc" }, take: 10 }),
        prisma.workOrder.count({ where: { orgId, status: { in: [WorkOrderStatus.OPEN, WorkOrderStatus.IN_PROGRESS] } } }),
        prisma.inventoryPart.count({ where: { orgId, quantityOnHand: { lte: 5 } } }),
        prisma.notification.count({ where: { orgId, recipientId: ctx.fleetopsUser.id, isRead: false } }),
        prisma.financialRecord.aggregate({ where: { orgId, type: "EXPENSE" }, _sum: { amount: true } }),
      ]);
  ...
  components: router({
    list: fleetOpsProcedure.input(z.object({ vehicleId: z.string().uuid().optional() }).optional()).query(({ ctx, input }) => prisma.component.findMany({ where: { vehicle: { orgId: ctx.fleetopsUser.orgId }, ...(input?.vehicleId ? { vehicleId: input.vehicleId } : {}) }, orderBy: { name: "asc" } })),
    create: fleetOpsProcedure.input(z.object({ vehicleId: z.string().uuid(), name: z.string().min(2), expectedLifeKm: z.number().positive(), lastServicedOdometer: z.number().nonnegative(), alertThresholdKm: z.number().positive() })).mutation(async ({ ctx, input }) => { requireRole(ctx.fleetopsUser.role, ["SUPERADMIN", "FLEET_MANAGER", "MECHANIC"]); assertWritable(ctx.fleetopsUser.org); const vehicle = await prisma.vehicle.findFirst({ where: { id: input.vehicleId, orgId: ctx.fleetopsUser.orgId } }); if (!vehicle) throw new TRPCError({ code: "NOT_FOUND", message: "Vehicle not found." }); return prisma.component.create({ data: input }); }),
    update: fleetOpsProcedure.input(z.object({ id: z.string().uuid(), name: z.string().min(2).optional(), expectedLifeKm: z.number().positive().optional(), lastServicedOdometer: z.number().nonnegative().optional(), alertThresholdKm: z.number().positive().optional() })).mutation(async ({ ctx, input }) => { requireRole(ctx.fleetopsUser.role, ["SUPERADMIN", "FLEET_MANAGER", "MECHANIC"]); assertWritable(ctx.fleetopsUser.org); const component = await prisma.component.findFirst({ where: { id: input.id, vehicle: { orgId: ctx.fleetopsUser.orgId } } }); if (!component) throw new TRPCError({ code: "NOT_FOUND", message: "Component not found." }); const { id, ...data } = input; return prisma.component.update({ where: { id }, data }); }),
    remove: fleetOpsProcedure.input(z.object({ id: z.string().uuid() })).mutation(async ({ ctx, input }) => { requireRole(ctx.fleetopsUser.role, ["SUPERADMIN", "FLEET_MANAGER"]); assertWritable(ctx.fleetopsUser.org); const component = await prisma.component.findFirst({ where: { id: input.id, vehicle: { orgId: ctx.fleetopsUser.orgId } } }); if (!component) throw new TRPCError({ code: "NOT_FOUND", message: "Component not found." }); return prisma.component.delete({ where: { id: input.id } }); }),
  }),
  vehicles: router({
    list: fleetOpsProcedure.query(({ ctx }) => prisma.vehicle.findMany({ where: { orgId: ctx.fleetopsUser.orgId }, include: { components: true }, orderBy: { updatedAt: "desc" } })),
    ...
    updateOdometer: fleetOpsProcedure.input(z.object({ vehicleId: z.string().uuid(), reading: z.number().min(0), source: z.enum(["MANUAL_DRIVER", "GPS_API", "MECHANIC"]) })).mutation(async ({ ctx, input }) => {
      assertWritable(ctx.fleetopsUser.org);
      const vehicle = await prisma.vehicle.findFirst({ where: { id: input.vehicleId, orgId: ctx.fleetopsUser.orgId } });
      ...
      const result = await prisma.$transaction([
        prisma.vehicle.update({ where: { id: vehicle.id }, data: { currentOdometer: input.reading } }),
        prisma.odometerLog.create({ data: { vehicleId: vehicle.id, driverId: ctx.fleetopsUser.id, reading: input.reading, source: input.source, isFlagged } }),
      ]);
  ...
  workOrders: router({
    list: fleetOpsProcedure.query(({ ctx }) => prisma.workOrder.findMany({ where: { orgId: ctx.fleetopsUser.orgId }, include: { vehicle: true, assignedMechanic: true, partsUsed: { include: { part: true } } }, orderBy: { createdAt: "desc" } })),
    create: fleetOpsProcedure.input(z.object({ vehicleId: z.string().uuid(), title: z.string().min(3), description: z.string().optional(), priority: z.nativeEnum(Priority).default(Priority.MEDIUM), assignedMechanicId: z.string().uuid().optional() })).mutation(async ({ ctx, input }) => {
      ...
      const vehicle = await prisma.vehicle.findFirst({ where: { id: input.vehicleId, orgId: ctx.fleetopsUser.orgId } });
      if (!vehicle) throw new TRPCError({ code: "NOT_FOUND", message: "Vehicle not found." });
      return prisma.workOrder.create({ data: { ...input, orgId: ctx.fleetopsUser.orgId } });
    }),
    complete: fleetOpsProcedure.input(z.object({ workOrderId: z.string().uuid(), parts: z.array(z.object({ partId: z.string().uuid(), qtyUsed: z.number().int().positive() })).default([]) })).mutation(async ({ ctx, input }) => {
      ...
      assertWritable(ctx.fleetopsUser.org);
      const order = await prisma.workOrder.findFirst({ where: { id: input.workOrderId, orgId: ctx.fleetopsUser.orgId }, include: { vehicle: true } });
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (admins.length) await tx.notification.createMany({ data: admins.map((admin) => ({ orgId: ctx.fleetopsUser.orgId, recipientId: admin.id, title: "Work order completed", message: `${order.vehicle.licensePlate} repair completed. Parts cost: ₹${partsCost.toLocaleString("en-IN")}.`, type: "WORK_ORDER_COMPLETE", referenceId: order.id })) });
  ...
  activity: router({
    recent: fleetOpsProcedure.query(async ({ ctx }) => {
      const [orders, alerts, odometers] = await Promise.all([
        prisma.workOrder.findMany({ where: { orgId: ctx.fleetopsUser.orgId }, include: { vehicle: true }, orderBy: { createdAt: "desc" }, take: 10 }),
        prisma.notification.findMany({ where: { orgId: ctx.fleetopsUser.orgId, recipientId: ctx.fleetopsUser.id }, orderBy: { createdAt: "desc" }, take: 10 }),
        prisma.odometerLog.findMany({ where: { vehicle: { orgId: ctx.fleetopsUser.orgId } }, include: { vehicle: true }, orderBy: { createdAt: "desc" }, take: 10 }),
      ]);
      return [
        ...orders.map((order) => ({ id: order.id, kind: "work_order", title: order.title, detail: `${order.vehicle.licensePlate} · ${order.status}`, createdAt: order.createdAt })),
        ...alerts.map((alert) => ({ id: alert.id, kind: "notification", title: alert.title, detail: alert.message, createdAt: alert.createdAt })),
        ...odometers.map((log) => ({ id: log.id, kind: "odometer", title: `Odometer updated · ${log.vehicle.licensePlate}`, detail: `${Number(log.reading).toLocaleString("en-IN")} km${log.isFlagged ? " · flagged" : ""}`, createdAt: log.createdAt })),
      ].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()).slice(0, 20);
    }),
  }),
  notifications: router({
    list: fleetOpsProcedure.query(({ ctx }) => prisma.notification.findMany({ where: { orgId: ctx.fleetopsUser.orgId, recipientId: ctx.fleetopsUser.id }, orderBy: { createdAt: "desc" }, take: 50 })),
    markRead: fleetOpsProcedure.input(z.object({ id: z.string().uuid() })).mutation(({ ctx, input }) => prisma.notification.updateMany({ where: { id: input.id, orgId: ctx.fleetopsUser.orgId, recipientId: ctx.fleetopsUser.id }, data: { isRead: true } })),
  }),
});
