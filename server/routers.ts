...
export const appRouter = router({
  ...
  audit: router({
    list: fleetOpsProcedure.input(z.object({ entityType: z.string().optional(), action: z.string().optional(), limit: z.number().int().min(1).max(200).default(100) }).optional()).query(({ ctx, input }) => { requireRole(ctx.fleetopsUser.role, ["SUPERADMIN"]); return fleetDb.auditEvent.findMany({ where: { orgId: ctx.fleetopsUser.orgId, ...(input?.entityType ? { entityType: input.entityType } : {}), ...(input?.action ? { action: input.action } : {}) }, orderBy: { createdAt: "desc" }, take: input?.limit ?? 100 }); }),
  }),
  automation: router({
