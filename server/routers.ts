...
export const appRouter = router({
  ...
  triage: router({
    queue: fleetOpsProcedure.query(async ({ ctx }) => { requireRole(ctx.fleetopsUser.role, ["SUPERADMIN", "FLEET_MANAGER"]); const cutoff = new Date(Date.now() + 30 * 86_400_000); const [issues, orders, documents, parts] = await Promise.all([fleetDb.vehicleIssue.findMany({ where: { orgId: ctx.fleetopsUser.orgId, status: { notIn: ["RESOLVED", "CLOSED"] } }, include: { vehicle: true }, orderBy: { createdAt: "desc" }, take: 50 }), fleetDb.workOrder.findMany({ where: { orgId: ctx.fleetopsUser.orgId, status: { notIn: ["COMPLETED", "CANCELLED"] } }, include: { vehicle: true, assignedMechanic: true }, orderBy: { createdAt: "desc" }, take: 50 }), fleetDb.document.findMany({ where: { orgId: ctx.fleetopsUser.orgId, expiryDate: { lte: cutoff } }, include: { vehicle: true }, orderBy: { expiryDate: "asc" }, take: 50 }), fleetDb.inventoryPart.findMany({ where: { orgId: ctx.fleetopsUser.orgId }, orderBy: { name: "asc" }, take: 200 })]); const issueItems = issues.map((item: any) => ({ id: item.id, kind: "VEHICLE_ISSUE", title: item.title, subtitle: `${item.vehicle?.licensePlate ?? item.vehicleId} · Driver issue`, priority: item.priority, status: item.status, createdAt: item.createdAt, referenceId: item.id, actionable: true })); const orderItems = orders.map((item: any) => ({ id: item.id, kind: "WORK_ORDER", title: item.title, subtitle: `${item.vehicle?.licensePlate ?? item.vehicleId} · ${item.assignedMechanic?.fullName ?? "Unassigned"}`, priority: item.priority, status: item.status, createdAt: item.createdAt, referenceId: item.id, actionable: true })); const documentItems = documents.map((item: any) => ({ id: item.id, kind: "DOCUMENT", title: item.title, subtitle: `${item.vehicle?.licensePlate ?? "Organization document"} · expires ${new Date(item.expiryDate).toLocaleDateString("en-IN")}`, priority: new Date(item.expiryDate) < new Date() ? "CRITICAL" : "HIGH", status: "REVIEW", createdAt: item.createdAt, referenceId: item.id, actionable: false })); const lowStockItems = parts.filter((item: any) => Number(item.quantityOnHand) <= Number(item.minReorderLevel)).map((item: any) => ({ id: item.id, kind: "LOW_STOCK", title: `${item.sku} · ${item.name}`, subtitle: `${item.quantityOnHand} on hand · reorder at ${item.minReorderLevel}`, priority: "HIGH", status: "REORDER", createdAt: item.updatedAt ?? item.createdAt, referenceId: item.id, actionable: false })); return [...issueItems, ...orderItems, ...documentItems, ...lowStockItems].sort((a: any, b: any) => { const rank: Record<string, number> = { CRITICAL: 0, HIGH: 1, MEDIUM: 2, LOW: 3 }; return (rank[a.priority] ?? 4) - (rank[b.priority] ?? 4) || new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(); }).slice(0, 100); }),
  }),
  audit: router({
