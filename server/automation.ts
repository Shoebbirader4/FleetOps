...
export async function evaluateVehicleMaintenance(vehicleId: string, orgId: string) {
  const vehicle = await prisma.vehicle.findFirst({ where: { id: vehicleId, orgId }, include: { components: true } });
  ...
  for (const component of vehicle.components) {
    ...
    const admins = await prisma.user.findMany({ where: { orgId, role: "SUPERADMIN" } });
    if (admins.length) {
...
export async function evaluateLowInventory(orgId: string) {
  const lowStock = await prisma.inventoryPart.findMany({ where: { orgId, quantityOnHand: { lte: 5 } } });
  if (!lowStock.length) return { lowStock: 0 };

  const admins = await prisma.user.findMany({ where: { orgId, role: { in: ["SUPERADMIN", "INVENTORY_MANAGER"] } } });
  for (const part of lowStock) {
    const alreadyNotified = await prisma.notification.findFirst({ where: { orgId, referenceId: part.id, type: "INVENTORY_LOW", createdAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } } });
    if (alreadyNotified) continue;
    if (admins.length) await prisma.notification.createMany({ data: admins.map((admin) => ({ orgId, recipientId: admin.id, title: "Inventory below reorder level", message: `${part.name} (${part.sku}) has ${part.quantityOnHand} units remaining.`, type: "INVENTORY_LOW", referenceId: part.id })) });
  }
...
export async function evaluateAllOrganizations() {
  ...
  for (const org of organizations) {
    const vehicles = await prisma.vehicle.findMany({ where: { orgId: org.id }, select: { id: true } });
