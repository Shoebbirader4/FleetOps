...
test("public landing exposes the two primary auth paths and a responsive visual artifact", async ({ page }, testInfo) => {
  ...
  expect(screenshot).toMatchSnapshot("landing.png", { maxDiffPixels: 30_000, maxDiffPixelRatio: 0.05 });
  await testInfo.attach("landing-responsive.png", { body: screenshot, contentType: "image/png" });
});
