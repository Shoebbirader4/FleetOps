...
export default function Home() {
  ...
  const persistedVehicles = useMemo(() => liveVehicles?.map((vehicle) => ({ id: vehicle.licensePlate, name: `${vehicle.make} ${vehicle.model} · ${vehicle.year}`, health: vehicle.status === "ACTIVE" ? 94 : 72, status: vehicle.status === "ACTIVE" ? "On route" : "At depot", odo: `${Number(vehicle.currentOdometer).toLocaleString("en-IN")} km`, service: "Live from FleetOps register", tone: vehicle.status === "ACTIVE" ? "good" : "warn" })) ?? vehicles, [liveVehicles]);
  const persistedOrders = useMemo(() => liveOrders?.map((order) => ({ id: order.id.slice(0, 8).toUpperCase(), title: order.title, vehicle: order.vehicle.licensePlate, owner: order.assignedMechanic?.fullName ?? "Unassigned", priority: order.priority[0] + order.priority.slice(1).toLowerCase(), due: order.status === "COMPLETED" ? "Completed" : "Open queue", status: order.status === "COMPLETED" ? "Completed" : "Open" })) ?? initialOrders, [liveOrders]);
  ...
  const visibleVehicles = useMemo(() => persistedVehicles.filter((vehicle) => `${vehicle.id} ${vehicle.name}`.toLowerCase().includes(query.toLowerCase()) && (filter === "All fleet" || vehicle.status === filter)), [query, filter]);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-canvas">
        <header className="topbar"><button className="mobile-menu" onClick={() => setShowMobileNav(true)} aria-label="Open navigation"><Menu size={19} /></button><div className="breadcrumb"><span>Avani Transit</span><span>/</span><strong>{activeNav}</strong></div><div className="topbar-actions"><button className="command-button" onClick={() => toast.info("Command palette", { description: "Try searching vehicles, work orders, or team members." })}><Command size={15} /> <span>Quick find</span><kbd>⌘ K</kbd></button><button className="notification-button" onClick={() => { setNotifications(0); toast.info(liveNotifications?.length ? `${liveNotifications.filter((item) => !item.isRead).length} live notifications` : "No live notifications"); }} aria-label="Notifications"><Bell size={18} />{session ? liveNotifications?.filter((item) => !item.isRead).length ? <i>{liveNotifications.filter((item) => !item.isRead).length}</i> : null : notifications > 0 && <i>{notifications}</i>}</button><div className="role-select-wrap"><button className="role-select" onClick={() => setShowRoleMenu(!showRoleMenu)}><span className="role-dot" /><span>{role.short}</span><ChevronDown size={14} /></button>{showRoleMenu && <div className="role-menu">{roles.map((item) => <button key={item.name} onClick={() => chooseRole(item)}><item.icon size={15} /><span>{item.name}</span>{role.name === item.name && <Check size={14} />}</button>)}<button className="role-menu-auth" onClick={() => session ? void handleSignOut() : toast.info("Supabase Auth required", { description: "Sign in with your FleetOps Supabase account to sync this workspace." })}>{session ? "Sign out" : "Sign in to sync"}</button></div>}</div></div></header>
