...
export default function Home() {
  ...
  const persistedOrders = useMemo(() => liveOrders?.map((order: any) => ({ id: order.id.slice(0, 8).toUpperCase(), sourceId: order.id, title: order.title, vehicle: order.vehicle.licensePlate, owner: order.assignedMechanic?.fullName ?? "Unassigned", priority: order.priority[0] + order.priority.slice(1).toLowerCase(), due: order.status === "COMPLETED" ? "Completed" : "Open queue", status: order.status === "COMPLETED" ? "Completed" : "Open" })) ?? initialOrders, [liveOrders]);
